import { users, minesGames, minesStats, minesLeaderboard, quizzes, quizQuestions, quizAttempts, quizLeaderboard, type User, type InsertUser, type MinesGame, type InsertMinesGame, type MinesStats, type InsertMinesStats, type MinesLeaderboard, type InsertMinesLeaderboard, type Quiz, type InsertQuiz, type QuizQuestion, type InsertQuizQuestion, type QuizAttempt, type InsertQuizAttempt, type QuizLeaderboard, type InsertQuizLeaderboard } from "../shared/schema";
import { db } from "./db";
...
export class DatabaseStorage implements IStorage {
  ...
  async updateQuiz(quizId: number, updates: Partial<Quiz>): Promise<Quiz | undefined> {
    const [quiz] = await db
      .update(quizzes)
  ...
  async getUserQuizAttempt(quizId: number, userJid: string): Promise<QuizAttempt | undefined> {
    const [attempt] = await db
      .select()
      .from(quizAttempts)
      .where(eq(quizAttempts.quizId, quizId))
      .where(eq(quizAttempts.userJid, userJid));
    return attempt || undefined;
  }
  ...
  async getQuizLeaderboard(quizId: number, limit: number = 10): Promise<QuizLeaderboard[]> {
    return await db
      .select()
      .from(quizLeaderboard)
      .where(eq(quizLeaderboard.quizId, quizId))
      .orderBy(asc(quizLeaderboard.rank))
      .limit(limit);
  }

  async updateQuizLeaderboard(quizId: number): Promise<void> {
    // Get completed attempts for this quiz
    const attempts = await db
      .select()
      .from(quizAttempts)
      .where(eq(quizAttempts.quizId, quizId))
      .where(eq(quizAttempts.isCompleted, true))
      .orderBy(desc(quizAttempts.totalScore));

    // Clear existing leaderboard
    await db.delete(quizLeaderboard).where(eq(quizLeaderboard.quizId, quizId));
    ...
    for (let i = 0; i < attempts.length; i++) {
      ...
      await db.insert(quizLeaderboard).values({
        ...
        rank: i + 1,
        balanceEarned: attempt.totalBalance,
        chipsEarned: attempt.totalChips,
