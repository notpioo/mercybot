    ...
    try {
        ...
        if (isGroup) {
            try {
                ...
                console.log(`🔍 Checking auto anti-view-once for group: ${remoteJid}`);
                console.log(`📋 Group settings antiViewOnce:`, group?.settings?.antiViewOnce);
                console.log(`📱 Message keys:`, Object.keys(msg || {}));
                
                if (group && group.settings.antiViewOnce) {
                    console.log(`🔄 Auto anti-view-once is enabled, processing message...`);
                    const viewOnceHandled = await handleAutoAntiViewOnce(sock, message, remoteJid, senderJid, senderName);
                    if (viewOnceHandled) {
                        console.log(`✅ View once message was handled automatically`);
                        return; // Stop processing if view once was handled
                    }
                } else {
                    console.log(`❌ Auto anti-view-once not enabled or group not found`);
                }
            } catch (error) {
