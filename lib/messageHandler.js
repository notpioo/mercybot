        ...
        if (isGroup) {
            try {
                ...
                if (group && group.settings.antiViewOnce) {
                    const viewOnceDetected = await checkViewOnce(msg, sock, remoteJid, senderJid, senderName, message);
                    if (viewOnceDetected) {
                        // Continue processing but don't return early
                    }
                }
...
async function checkBadWords(text, badWords, sock, remoteJid, senderJid, senderName, message) {
    try {
        ...
        const foundBadwords = badWords.filter(badword => 
            textLower.includes(badword.toLowerCase())
        );
        ...
        const warningMessage = `⚠️ *BADWORD DETECTED*\n\n` +
            ...
            `⚠️ Warning: ${newWarningCount}/${config.userSystem.maxWarnings}\n\n` +
...
async function checkViewOnce(messageContent, sock, remoteJid, senderJid, senderName, message) {
    try {
        // Check if message contains view once media
        const hasViewOnce = messageContent.imageMessage?.viewOnce || 
                           messageContent.videoMessage?.viewOnce ||
                           messageContent.audioMessage?.viewOnce;

        if (!hasViewOnce) {
            return false; // No view once detected
        }

        console.log(`🔍 View once detected from ${senderName} in group`);

        // Download and resend the media without view once
        let mediaMessage = null;
        let mediaType = '';
        let caption = '';

        if (messageContent.imageMessage?.viewOnce) {
            mediaMessage = messageContent.imageMessage;
            mediaType = 'image';
            caption = mediaMessage.caption || '';
        } else if (messageContent.videoMessage?.viewOnce) {
            mediaMessage = messageContent.videoMessage;
            mediaType = 'video';
            caption = mediaMessage.caption || '';
        } else if (messageContent.audioMessage?.viewOnce) {
        ...
        if (mediaMessage) {
            try {
                ...
                const resendMessage = {
                    ...
                    caption: `🔄 *ANTI VIEW ONCE DETECTED*\n\n` +
                            `👤 Original sender: @${senderJid.split('@')[0]}\n` +
                            `📝 Original caption: ${caption || 'Tidak ada caption'}\n\n` +
                            `⚠️ Pesan view once telah dikirim ulang oleh bot`,
                    mentions: [senderJid]
                };
                ...
                console.log(`📸 Successfully resent ${mediaType} from ${senderName} without view once`);
                
                return true; // View once was detected and processed

            } catch (downloadError) {
                console.error('❌ Failed to download or resend view once media:', downloadError);
                
                // Send notification that view once was detected but couldn't be resent
                await sock.sendMessage(remoteJid, {
                    text: `🔄 *ANTI VIEW ONCE DETECTED*\n\n` +
                          `👤 @${senderJid.split('@')[0]} mengirim ${mediaType} view once\n` +
                          `❌ Gagal mengunduh media untuk dikirim ulang`,
                    mentions: [senderJid]
                });
            }
        }
        ...
        console.error('❌ Error in view once detection:', error);
        return false;
    }
}
